...
const NO_MATCHING_ACTION = request => {
  throw {
    ...
    message: `Could not find matching action for ${request.path} and method ${
      request.httpMethod
    }`
  };
};
...
function addCorsHeaders(toAdd) {
  ...
  toAdd["Access-Control-Allow-Headers"] =
    "Content-Type,X-Amz-Date,Authorization,X-Api-Key,X-Amz-Security-Token";
  return toAdd;
}

function processActionAndReturn(
  actionConfig,
  event,
  context,
  headers,
...
function convertError(error, errorMapping, headers) {
  ...
  return {
    ...
    body: JSON.stringify({
      error: "ServerError",
      message: `Generic errors: ${JSON.stringify(
...
function extractPathValues(pathExpression, httpPath) {
  const pathValueRegex = new RegExp(
    "^" + pathExpression.replace(/:[\w]+/g, "([^/]+)") + "$"
  );
